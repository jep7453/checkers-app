...
| As a player, I want to be able to move checker pieces diagonally so that I can take a turn. | **Given** when I move a checker piece  in the adjacent square **then** the piece moves away from me.|||PASS|JP 4-5-2020||  
|   |**Given** when I move a checker piece in the adjacent square **then** the piece occupies the valid back square.|||PASS|JP 4-5-2020|||  
|   |**Given** I move a piece and it is a king **then** it can move backwards and forwards|||PASS|JP 4-5-2020||||  
| As a player I want to jump an opposing piece so that I can win the game. | Given a player jumps a piece of opposing color, when the player's piece is adjacent to the opposing piece, then the opposing piece should be deleted|||PASS|JP 4-5-2020||  
|   |Given that a player is NOT a king, when trying to jump a piece, then that opposing piece must be ahead of the player piece.|||PASS|JP 4-5-2020||||  
|   |Given that a player IS a king, when trying to jump a piece, then that opposing piece can be ahead or behind of the player piece.|||PASS|JP 4-5-2020||||  
| As a player I want to eliminate multiple pieces so that I can win the game. |**Given** that a player's piece is NOT a King and is adjacent to an opposing piece, which is adjacent to an empty valid space, which is adjacent to an opposing piece, which is adjacent to an open valid space (which all have to be forward) **when** the player moves their piece to the last valid space next to an adjacent piece, **then** I expect all jumped pieces to be deleted. |||PASS|JP 4-5-2020||  
...
| As a player, I want my piece promoted to a king if it reaches the other side of the board so that I can move it backwards. |**Given** I have a Checker **when** I move it to the last row **then** it will turn into a King|||PASS|JP 4-5-2020||  
|   |**Given** I have a King **when** I try to move it **then** it will be able to move backwards as well.|||PASS|JP 4-5-2020|||  
| As a player, I want to sign-out so that I can stop playing checkers. |**Given** I am signed **when** I am on the home page and not in a game **then** I should see a means to sign out.|||PASS|JP 4-5-2020||  
|   |**Given** I am signed **when** I am in a game **then** I should see a means to sign out and resign the game when I do.|||PASS|JP 4-5-2020|||  
| As a player, I want the game to end if I cannot make a move so that my opponent can be declared the winner. |**Given** I am in a game and it is my turn, **when** I have no legal moves **then** I expect to lose the game.|||PASS|JP 4-5-2020||  
